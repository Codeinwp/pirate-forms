	...
	function build_form( $echo = true ) {
		...
		foreach ( $this->inputs as $val ) :
			...
			switch ( $val['type'] ) {
				...
				case 'textarea':
					$element = 'textarea';
					$end     = ' class="form-control" placeholder="'.$val['placeholder'].'">' . htmlspecialchars( $val['value'] ) . '</textarea>';
					break;
				...
				default :
					...
					if( !empty($val['type']) && ($val['type'] == 'hidden' ) ) {
						$end .= ' class="form-control" type="' . $val['type'] . '" value="' . htmlspecialchars ( $val['value'] ) . '"';
					}
					else {
						$end .= ' class="form-control" type="' . $val['type'] . '" value="' . htmlspecialchars ( $val['value'] ) . '" placeholder="' . $val['placeholder'] . '"';
					}

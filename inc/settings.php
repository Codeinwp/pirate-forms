...
function pirate_forms_admin() {
	...
	<div class="wrap">
		...
		<div class="pirate-subscribe postbox card">
			<h3 class="title"><?php esc_html_e( 'Subscribe', 'islemag' )?></h3>
			<div class="pirate-forms-subscribe-content">
				<?php
				      ...
				      if( $was_submited == false ){
				        echo sprintf( '<p> %s </p><form class="pirate-forms-submit-mail" method="post"><input name="pirate_forms_mail" type="email" value="'.get_option( 'admin_email' ) .'" /><input class="button" type="submit" value="Submit"></form>', esc_html__('Our free, 4-lesson course on how to make your WordPress site run incredibly fast is barely waiting for its students. Ready to learn how to reduce your loading times by half? Come and join the 1st lesson here!', 'pirate-forms' ) );
				      } else {
...
function pirate_forms_dashboard_widget() {
	wp_add_dashboard_widget( 'pirate_forms_subscribe_widget', __( 'Subscribe', 'pirate-forms' ),'pirate_forms_widget_content');
}
add_action( 'wp_dashboard_setup', 'pirate_forms_dashboard_widget' );

function pirate_forms_widget_content() {
